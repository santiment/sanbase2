  ...
  node('sanbase-builder') {
    stage('Run Tests') {
      container('docker') {
        ...
        try {
          sh "docker run --rm --link postgres_${env.BRANCH_NAME}:db --link influxdb_${env.BRANCH_NAME}:influxdb --env DATABASE_URL=postgres://postgres:password@db:5432/postgres,INFLUXDB_HOST=http://influxdb:8086 -t sanbase-test:${env.BRANCH_NAME}"
        } finally {
