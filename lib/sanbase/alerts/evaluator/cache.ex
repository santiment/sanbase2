defmodule Sanbase.Alert.Evaluator.Cache do
  @moduledoc ~s"""
  Cache that is used during custom user alerts evaluation. A subset of the
  alert's settings that uniquely determine the outcome are usead to generate a sha256
  hash so 2 alerts with the same settings are going to be executed only once.

  The TTL of the cache is small (3 minutes) with 1 minute checks so it will expire
  before the alerts are scheduled again (5 minutes).
  """

  require Logger
  @cache_name :alerts_evaluator_cache
