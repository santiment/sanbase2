defmodule Sanbase.Billing.StripeEvent do
  ...
  # A bundle cannot take the legacy path: that reads item[0]'s `plan` and looks it up in
  # `plans`, while a bundle's item prices live only in `bundle_prices`. On stage it left the
  # event `is_processed: false` with "Plan for subscription_id ... does not exist", hence
  # this branch first. The legacy clause below is unchanged.
  ...
  # Normally the row already exists, created by `Bundle.Lifecycle.subscribe/2` before
  # Stripe's event arrived. `ItemSync` decides between reconciling it and adopting a
  # subscription assembled outside our flow, refusing to adopt one whose item prices are
  # not all in the bundle catalog.
  ...
  # Everything the status sync cannot do itself, for the two event types needing it.
  # `invoice.payment_succeeded`/`payment_failed` also reach `handle_event_common/3` and are
  # left alone - a renewal does not change what was bought. `subscription` is the row as
  # read before the sync, which writes only `plan_id` and the period fields; `fetch_plan_id/2`
  # cannot move a bundle off its marker plan, so the plan branched on here is the same
  # either way.
  ...
  # The items and the entitlement stay: the status already denies access, and support
  # answers "what did I pay for" from these rows. Only the quota row is recomputed - it
  # caches resolved numbers that would keep granting a canceled bundle's allowance.
