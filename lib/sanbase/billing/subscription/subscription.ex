defmodule Sanbase.Billing.Subscription do
  ...
  # Only the new offering (Institutional, Enterprise) is serialized.
  # `ensure_plan_is_for_sale/2` reads the customer's live SanAPI subscriptions and
  # then charges, so without a lock two overlapping requests both pass that read and
  # both create a billable Stripe subscription - see `Subscription.PurchaseLock`.
  # `Bundle.Lifecycle.subscribe/2` takes the same lock, so all three flows exclude
  # each other per user.
  #
  ...
  defp serialize_new_offering_purchase(user, %Plan{name: "ENTERPRISE" <> _}, fun) do
    __MODULE__.PurchaseLock.with_lock(user.id, fun)
  end
