defmodule Sanbase.Clickhouse.ApiCallData do
  ...
  def active_users_count(from, to) do
    ...
    query_struct
    |> ClickhouseRepo.query_transform(fn [value] -> value end)
    |> maybe_unwrap_ok_value()
  end
  ...
  def api_metric_distribution do
    ...
    query_struct
    |> ClickhouseRepo.query_transform(fn [metric, count] ->
  ...
  def api_metric_distribution_per_user do
    ...
    query_struct
    |> ClickhouseRepo.query_reduce(%{}, fn [user_id, metric, count], acc ->
      Map.update(acc, user_id, %{}, fn map ->
        update_api_distribution_user_map(map, metric, count)
      end)
    end)
    |> maybe_sort(:count, :desc)
    |> maybe_apply_function(&Enum.map(&1, fn {user_id, map} -> Map.put(map, :user_id, user_id) end))
  end
