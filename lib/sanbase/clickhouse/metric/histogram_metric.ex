defmodule Sanbase.Clickhouse.MetricAdapter.HistogramMetric do
  ...
  def histogram_data("age_distribution" = metric, %{slug: slug}, from, to, interval, limit) do
    ...
    ClickhouseRepo.query_transform(query_struct, fn [unix, value] ->
      range_from = DateTime.from_unix!(unix)

      range_to = Enum.min_by([Timex.shift(range_from, seconds: str_to_sec(interval)), to], &DateTime.to_unix/1)
  ...
  def histogram_data(metric, %{slug: slug}, from, to, interval, limit) when metric in @spent_coins_cost_histograms do
    query_struct = histogram_data_query(metric, slug, from, to, interval, limit)

    query_struct
    |> ClickhouseRepo.query_transform(fn [price, amount] ->
      %{
  ...
  def first_datetime(metric, %{slug: slug}, opts) when metric in @spent_coins_cost_histograms do
  ...
  def first_datetime(metric, _selector, _opts) when metric in @eth2_metrics do
  ...
  def last_datetime_computed_at(metric, %{slug: slug}, opts) when metric in @spent_coins_cost_histograms do
    with {:ok, dt1} <-
  ...
  defp maybe_transform_into_buckets({:error, error}, _slug, _from, _to, _limit), do: {:error, error}
  ...
  defp do_transform_to_buckets(prices_list, ranges_map, min, bucket_size, price_break) do
    ...
    prices_list
    |> Enum.reduce(ranges_map, fn %{price: price, value: value}, acc ->
  ...
  defp get_min_max_prices(prices_list) do
    ...
    {min, max} = prices_list |> Enum.map(& &1.price) |> Sanbase.Math.min_max()
  ...
  defp ranges_map(min, buckets_count, bucket_size) do
    ...
    min
    |> Stream.unfold(price_ranges)
    |> Enum.take(buckets_count)
    |> Map.new(fn range -> {range, 0.0} end)
  end
  ...
  defp split_bucket_containing_price(bucketed_data, original_data, [low, high], divider) do
    {lower_half_amount, upper_half_amount} =
      Enum.reduce(original_data, {0.0, 0.0}, fn %{price: price, value: value}, {acc_lower, acc_upper} ->
        cond do
