defmodule Sanbase.ExternalServices.Etherscan.Scraper do
  ...
  def fetch_address_page(address) do
    case get("/address/#{address}") do
      ...
      {:ok, %Tesla.Env{status: status}} ->
        Logger.warning("Invalid response from etherscan for address #{address}. Status: #{status}")

        nil

      {:error, error} ->
        Logger.warning("Error response from etherscan for address #{address}. Reason: #{inspect(error)}")
  ...
  def fetch_token_page(token_name) do
    case get("/token/#{token_name}") do
      ...
      {:error, error} ->
        Logger.warning("Error response from etherscan for #{token_name}. Reason: #{inspect(error)}")
  ...
  def parse_token_page!(html, project_info) do
    ...
    %ProjectInfo{
      ...
      | # We want to override the currently stored total_supply and that's the reason why the order is different than the rest of the fields
        ...
        email: project_info.email || html |> official_link("Email") |> email(),
  ...
  defp website_link(html) do
    html
    |> Floki.find(~s/#ContentPlaceHolder1_tr_officialsite_1 > div > div.col-md-8 > a/)
  ...
  defp official_link(html, media) do
    html
    |> Floki.find(~s/a[data-original-title^="#{media}:"]/)
    |> Floki.attribute("href")
    |> List.first()
  end
  ...
  defp creation_transaction(html) do
    html
    |> Floki.find(~s/a[title="Creator TxHash"]/)
    |> List.first()
    |> case do
      nil -> nil
  ...
  defp total_supply(html) do
    ...
    html
    |> Floki.find(~s/#ContentPlaceHolder1_divSummary > div:first-child  div > div + div/)
    |> Enum.at(0)
    |> case do
      nil ->
        nil
  ...
  defp main_contract_address(html) do
    html
    |> Floki.find(~s/div:fl-contains('Contract') + div/)
    |> List.first()
    |> case do
      nil -> nil
      match -> Floki.text(match)
  ...
  defp token_decimals(html) do
    html
    |> Floki.find(~s/div:fl-contains('Decimals') + div/)
    |> List.first()
    |> case do
      nil -> nil
      match -> match |> Floki.text() |> parse_token_decimals()
    end
  end
  ...
  defp parse_total_supply(total_supply) do
    ...
    |> then(fn supply -> String.trim(supply, "Supply:") end)
    |> D.new()
  end
end
