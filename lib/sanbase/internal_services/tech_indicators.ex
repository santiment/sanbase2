defmodule Sanbase.InternalServices.TechIndicators do
    ...
    def macd(ticker, currency, from_datetime, to_datetime, aggregate_interval) do
      ...
      url = "#{tech_indicators_url()}/indicator/macd?ticker=#{ticker}&currency=#{currency}&from_timestamp=#{from_unix}&to_timestamp=#{to_unix}&aggregate_interval=#{aggregate_interval}"
    ...
    def rsi(ticker, currency, from_datetime, to_datetime, aggregate_interval, rsi_interval) do
      ...
      url = "#{tech_indicators_url()}/indicator/rsi?ticker=#{ticker}&currency=#{currency}&from_timestamp=#{from_unix}&to_timestamp=#{to_unix}&aggregate_interval=#{aggregate_interval}&rsi_interval=#{rsi_interval}"
    ...
    def price_volume_diff(ticker, currency, from_datetime, to_datetime, aggregate_interval) do
      ...
      url = "#{tech_indicators_url()}/indicator/pricevolumediff?ticker=#{ticker}&currency=#{currency}&from_timestamp=#{from_unix}&to_timestamp=#{to_unix}&aggregate_interval=#{aggregate_interval}"
