defmodule Sanbase.Metric.Category.MetricCategoriesMapping do
  @moduledoc """
  ...
  schema "metric_categories_mapping" do
    field(:metric_registry_id, :integer)
    field(:module, :string)
    field(:metric, :string)
  ...
  def changeset(%__MODULE__{} = mapping, attrs) do
    mapping
    |> cast(attrs, [:metric_registry_id, :module, :metric, :group_id])
  ...
  """
  @spec list_with_preloads() :: [t()]
  def list_with_preloads do
    query =
      from(m in __MODULE__,
        preload: [:group, :metric_registry]
      )
  ...
  def create_by_metric_registry_id(metric_registry_id, group_id)
      when is_integer(metric_registry_id) and is_integer(group_id) do
    create(%{
  ...
  @spec create_by_module_and_metric(String.t(), String.t(), integer()) ::
          {:ok, t()} | {:error, Ecto.Changeset.t()}
  def create_by_module_and_metric(module, metric, group_id)
      when is_binary(module) and is_binary(metric) and is_integer(group_id) do
    create(%{
      module: module,
      metric: metric,
      group_id: group_id
  ...
  defp valid_metric_reference?(metric_registry_id, module, metric) do
    ...
    (not is_nil(metric_registry_id) and is_nil(module) and is_nil(metric)) or
      (is_nil(metric_registry_id) and not is_nil(module) and not is_nil(metric))
  end
