defmodule Sanbase.Metric.Category.MetricGroup do
  ...
  @type t :: %__MODULE__{
          ...
          mappings: [MetricCategoriesMapping.t()],
  ...
  schema "metric_groups" do
    ...
    has_many(:mappings, MetricCategoriesMapping, foreign_key: :group_id)
  ...
  def changeset(%__MODULE__{} = group, attrs) do
    group
    |> cast(attrs, [:name, :slug, :short_description, :description, :display_order, :category_id])
    |> validate_required([:name, :slug, :category_id])
    ...
    |> unique_constraint([:slug, :category_id])
  end
  ...
  """
  @spec get_by_slug_and_category(String.t(), integer()) :: t() | nil
  def get_by_slug_and_category(slug, category_id)
      when is_binary(slug) and is_integer(category_id) do
    Repo.get_by(__MODULE__, slug: slug, category_id: category_id)
  end
  ...
  """
  @spec create_if_not_exists(String.t(), String.t(), integer()) :: {:ok, t()}
  def create_if_not_exists(name, slug, category_id)
      when is_binary(name) and is_binary(slug) and is_integer(category_id) do
    case get_by_slug_and_category(slug, category_id) do
      nil ->
        case create(%{name: name, slug: slug, category_id: category_id}) do
          {:ok, group} -> {:ok, group}
  ...
  defp update_group_order(%{id: id, display_order: display_order}) do
    case get(id) do
      ...
      group ->
        case update(group, %{display_order: display_order}) do
          {:ok, _} -> :ok
