defmodule Sanbase.Metric.MetricReplace do
  ...
  def maybe_replace_metric("price_usd", %{slug: slug})
      when slug in @slugs_with_changed_price_metric,
      do: "price_usd_5m"

  def maybe_replace_metric("volume_usd", %{slug: slug})
      when slug in @slugs_with_changed_volume_metric,
      do: "volume_usd_5m"

  def maybe_replace_metric(metric, _selector) when is_binary(metric), do: metric

  def maybe_replace_metrics([_ | _] = metrics_list, %{slug: slug} = selector)
      when slug in @slugs_with_changed_metric do
