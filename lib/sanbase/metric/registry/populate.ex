defmodule Sanbase.Metric.Registry.Populate do
  ...
  def json_map_to_registry_params(%{} = map) do
    ...
    %{
      ...
      aliases: map |> Map.get("aliases", []) |> Enum.map(&%{name: &1}),
      data_type: map["data_type"],
      deprecation_note: map["deprecation_note"],
      docs: map |> Map.get("docs_links", []) |> Enum.map(&%{link: &1}),
      ...
      required_selectors: map |> Map.get("required_selectors", []) |> Enum.map(&%{type: &1}),
      selectors: map |> Map.get("selectors", []) |> Enum.map(&%{type: &1}),
  ...
  def json_map_to_registry_changeset(%{} = map) do
    ...
    Sanbase.Metric.Registry.changeset(%Sanbase.Metric.Registry{}, params)
  end
  ...
  defp process_metrics do
    Enum.reduce_while(Sanbase.Clickhouse.MetricAdapter.FileHandler.raw_metrics_json(), [], &process_single_metric/2)
  end
  ...
  defp handle_metric_record({:ok, record}, map) do
    ...
    new_record = Ecto.Changeset.apply_changes(changeset)
