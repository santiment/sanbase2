defmodule Sanbase.Model.Project do
    ...
    all_icos_query =
      from(
        ...
        select: %{
          ...
          rank:
            fragment("row_number() over(partition by ? order by ? asc)", i.project_id, i.start_date)
        }
      )
  ...
  defp fill_missing_tokens_sold_at_icos(project) do
    with tokens_sold_at_icos <- Enum.map(project.icos, &(&1.tokens_sold_at_ico)),
      ...
      known_tokens_sum = tokens_sold_at_icos
      |> Enum.reject(&is_nil/1)
      |> Enum.reduce(zero, &Decimal.add/2)

      unknown_tokens_sum = Decimal.compare(project.latest_coinmarketcap_data.available_supply, known_tokens_sum)
      |> case do
        ^one -> Decimal.sub(project.latest_coinmarketcap_data.available_supply, known_tokens_sum)
        _ -> zero
      end
