defmodule Sanbase.Project.Selector do
  ...
  def args_to_raw_selector(%{selector: %{slugs: _} = selector}), do: Sanbase.MapUtils.rename_key(selector, :slugs, :slug)

  def args_to_raw_selector(%{selector: %{} = selector}), do: selector
  def args_to_raw_selector(_), do: %{}

  def args_to_selector(args) do
    selector = args_to_raw_selector(args)
  ...
  defp valid_selector?(%{slug: slug}) when is_binary(slug) do
    if @available_slugs_module.valid_slug?(slug) do
      true
    else
      {:error, "The slug #{inspect(slug)} is not an existing slug."}
    end
  end
  ...
  defp valid_selector?(%{slug: slugs}) when is_list(slugs) and length(slugs) < 10 do
    ...
    Enum.find_value(slugs, true, fn slug ->
      if @available_slugs_module.valid_slug?(slug) do
        false
      else
        {:error, "The slug #{inspect(slug)} is not an existing slug."}
  ...
  defp transform_selector(%{market_segments: market_segments} = selector) do
    slugs =
      market_segments
      |> Sanbase.Project.List.by_market_segment_all_of()
      |> Enum.map(& &1.slug)
  ...
  defp transform_selector(%{watchlist_id: watchlist_id} = selector) when is_integer(watchlist_id) do
  ...
  defp transform_selector(%{watchlist_slug: watchlist_slug} = selector) when is_binary(watchlist_slug) do
