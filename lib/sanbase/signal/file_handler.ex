defmodule Sanbase.Signal.FileHandler do
  ...
  defmodule Helper do
    ...
    def resolve_timebound_signals(signal_map, timebound_values) do
      ...
      Enum.map(timebound_values, fn timebound ->
    ...
    def atomize_access_level_value(access) when is_binary(access), do: String.to_existing_atom(access)

    def atomize_access_level_value(access) when is_map(access) do
      Map.new(access, fn {k, v} -> {k, String.to_existing_atom(v)} end)
    end
  ...
  @signals_json_pre_timebound_expand available_signals_file |> File.read!() |> Jason.decode!()
  ...
  @access_map Helper.name_to_field_map(@signals_json, "access", transform_fn: &Helper.atomize_access_level_value/1)
  @table_map Helper.name_to_field_map(@signals_json, "table", required?: true)
  @aggregation_map Helper.name_to_field_map(@signals_json, "aggregation", transform_fn: &String.to_atom/1)
  ...
  @data_type_map Helper.name_to_field_map(@signals_json, "data_type", transform_fn: &String.to_atom/1)

  @signals_list Enum.map(@signals_json, fn %{"name" => name} -> name end)
  ...
  @min_plan_map Helper.name_to_field_map(@signals_json, "min_plan",
                  transform_fn: fn plan_map ->
                    Map.new(plan_map, fn {k, v} -> {k, String.upcase(v)} end)
                  end
                )

  @signals_data_type_map Helper.name_to_field_map(@signals_json, "data_type", transform_fn: &String.to_atom/1)
  ...
  @signals_json_pre_timebound_expand
  |> Enum.group_by(fn signal -> {signal["signal"], signal["data_type"]} end)
  |> Map.values()
