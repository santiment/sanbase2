defmodule Sanbase.Signal.SignalAdapter do
  ...
  @signals Enum.to_list(@signals_mapset)
  ...
  def has_signal?(signal) do
    if signal in @signals_mapset do
      true
    else
  ...
  def access_map do
    Map.new(@access_map, fn {signal, restrictions} ->
  ...
  def available_signals(%{slug: slug}) when is_binary(slug) do
    ...
    query_struct
    |> ClickhouseRepo.query_transform(fn [signal] ->
  ...
  def raw_data(signals, selector, from, to) do
    ...
    query_struct
    |> ClickhouseRepo.query_transform(fn [unix, signal, slug, value, metadata] ->
      metadata =
