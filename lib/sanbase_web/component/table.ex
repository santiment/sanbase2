defmodule SanbaseWeb.TableComponent do
      ...
      <tbody class="text-sm font-normal text-gray-700">
        <%= for row <- @rows do %>
          <tr class="hover:bg-gray-100 border-b border-gray-200 py-4">
            <%= for field <- @fields do %>
              <.td value={if @funcs[field] != nil, do: @funcs[field].(row), else: Map.get(row, field)}  />
            <% end %>
  ...
  def a(assigns) do
    ~H"""
    <.link href={Routes.generic_path(SanbaseWeb.Endpoint, @action, @row, resource: @resource)} class="underline"><%= Atom.to_string(@action) %></.link>
    """
  end
  ...
  def search(assigns) do
    ...
    <.form :let={f}, for={:search}, action={Routes.generic_path(SanbaseWeb.Endpoint, :search, resource: @resource)} >
      <%= text_input f, :generic_search, value: @search_value, class: "w-32 pl-10 pr-4 text-indigo-600 border-gray-200 rounded-md sm:w-64 focus:border-indigo-600 focus:ring focus:ring-opacity-40 focus:ring-indigo-500"%>
      <%= submit "Search" %>
    </.form>
    </div>
    """
