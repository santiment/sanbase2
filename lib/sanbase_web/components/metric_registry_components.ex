    ...
    <div class="flex flex-col break-normal">
      <div class="text-base-content text-base">
        ...
        <span
          ...
          class={if @status == "alpha", do: "text-warning text-sm", else: "text-secondary text-sm"}
        >
          ({String.upcase(@status)})
        </span>
      </div>
      <div class="text-base-content/80 text-sm">{@metric} (API)</div>
      <div class="text-base-content/80 text-sm">{@internal_metric} (DB)</div>
      <div :if={@category_mappings != []} class="flex flex-wrap gap-1 mt-1">
        <span
          :for={category_mapping <- @category_mappings}
          class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-info/20 text-info"
        >
