defmodule SanbaseWeb.Graphql.Complexity.TechIndicatorsComplexity do
    ...
    defp get_complexity(from_datetime, to_datetime, interval) do
        ...
        |> case do
            nil -> 1_000_000 # should be larger than the max complexity
            intervals_count -> Kernel.trunc(intervals_count * 5)
        end
    end
    ...
    defp get_interval_seconds(interval) do
        with %{"number" => number_str, "unit" => unit} <- Regex.named_captures(~r/^(?<number>\d+)(?<unit>d|h|m|s)$/, interval),
