defmodule SanbaseWeb.Graphql.Resolvers.MetricResolver do
  ...
  alias Sanbase.Metric.CatchableError
  ...
  def get_executed_clickhouse_sql(_root, _args, _resolution) do
    {:ok, :__executed_clickhouse_sql_list__ |> Process.get([]) |> Enum.reverse()}
  end
  ...
  def get_available_metrics(_root, %{plan: plan, product: product} = args, _resolution) do
    ...
    metrics =
      plan_name
      |> AccessChecker.get_available_metrics_for_plan(product_code)
  ...
  def get_available_slugs(_root, _args, %{source: %{metric: metric}}), do: Metric.available_slugs(metric)
  ...
  def get_human_readable_name(_root, _args, %{source: %{metric: metric}}), do: Metric.human_readable_name(metric)
  ...
  def timeseries_data_complexity(_root, args, resolution) do
    ...
    complexity =
      args
      |> SanbaseWeb.Graphql.Complexity.from_to_interval(child_complexity, resolution)
  ...
  def available_since(_root, args, %{source: %{metric: metric}}) do
    with_result =
      with {:ok, selector} <- args_to_selector(args),
           ...
           {:ok, opts} <- selector_args_to_opts(args) do
        Metric.first_datetime(metric, selector, opts)
      end
  ...
  def broken_data(_root, %{from: from, to: to} = args, %{source: %{metric: metric}}) do
    with_result =
      with {:ok, selector} <- args_to_selector(args),
           ...
           true <- valid_owners_labels_selection?(args) do
        Metric.broken_data(metric, selector, from, to)
      end
  ...
  def timeseries_data(_root, args, %{source: %{metric: metric}} = resolution) do
    ...
    e in [CatchableError] -> {:error, Exception.message(e)}
  ...
  def aggregated_timeseries_data(_root, %{from: from, to: to} = args, %{source: %{metric: metric}}) do
  ...
  def histogram_data(_root, args, %{source: %{metric: metric}}) do
  ...
  def table_data(_root, %{from: from, to: to} = args, %{source: %{metric: metric}}) do
    with_result =
      with {:ok, selector} <- args_to_selector(args),
           true <- all_required_selectors_present?(metric, selector),
           true <- valid_metric_selector_pair?(metric, selector),
           true <- valid_owners_labels_selection?(args) do
        Metric.table_data(metric, selector, from, to)
      end
  ...
  def latest_metrics_data(_root, %{metrics: metrics} = args, _resolution) do
    with {:ok, selector} <- args_to_selector(args),
         :ok <- check_metrics_slugs_cartesian_limit(metrics, selector, 20_000) do
      Metric.LatestMetric.latest_metrics_data(metrics, selector)
    end
  end
  ...
  defp all_required_selectors_present?(metric, selector) do
    ...
    with {:ok, required_selectors} <- Metric.required_selectors(metric) do
  ...
  defp do_check_required_selectors(metric, selector_names, required_selectors) do
    Enum.reduce_while(required_selectors, true, fn list, acc ->
      if Enum.any?(list, &(&1 in selector_names)) do
        {:cont, acc}
      else
        selectors_str =
          list
          |> Enum.map(&Atom.to_string/1)
          |> Enum.map_join(", ", &Inflex.camelize(&1, :lower))
  ...
  defp check_metrics_slugs_cartesian_limit(metrics, %{slug: slug_or_slugs}, limit) do
    ...
    if cartesian_product_length <= limit do
      :ok
    else
      {:error, "The length of the metrics multiplied by the length of\
  ...
  defp maybe_enrich_with_labels(_metric, [%{address: address} | _] = data) when is_binary(address) do
    addresses = data |> Enum.map(& &1.address) |> Enum.uniq()

    {:ok, labels} = Sanbase.Clickhouse.Label.get_address_labels("ethereum", addresses)

    labeled_data =
      Enum.map(data, fn %{address: address} = elem ->
        address_labels = labels |> Map.get(address, []) |> Enum.map(& &1.name)
  ...
  defp transform_interval("all_spent_coins_cost", interval) do
    [Sanbase.DateTimeUtils.str_to_days(interval), 1]
    |> Enum.max()
    |> to_string()
    |> Kernel.<>("d")
  end
  ...
  defp valid_metric_selector_pair?("social_active_users", selector) when not has_binary_key?(selector, :source) do
    {:error,
     """
  ...
  defp maybe_filter_incomplete_metrics(metrics, true = _has_incomplete_data) do
    ...
    incomplete_metrics
    |> MapSet.new()
    |> MapSet.intersection(MapSet.new(metrics))
    |> Enum.to_list()
  end
  ...
  defp remove_hidden_metrics(metrics) do
    ...
    Enum.reject(metrics, &(&1 in hidden_metrics))
  end
end
