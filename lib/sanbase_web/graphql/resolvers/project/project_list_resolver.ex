defmodule SanbaseWeb.Graphql.Resolvers.ProjectListResolver do
  ...
  # defp post_fetch_projects_processing(projects, opts) do
  #   projects_map = projects |> Enum.into(%{}, fn p -> {p.slug, p} end)

  #   projects =
  #     case opts[:has_order?] do
  #       true ->
  #         opts[:ordered_slugs]
  ...
  #   projects =
  #     case opts[:has_pagination?] do
  #       true ->
  #         page = opts[:pagination][:page] |> IO.inspect(label: "103", limit: :infinity)
  #         page_size = opts[:pagination][:page_size] |> IO.inspect(label: "104", limit: :infinity)

  #         projects |> Enum.drop((page - 1) * page_size) |> Enum.take(page_size)

  #       false ->
  #         projects
  #     end

  #   projects |> Enum.reject(&is_nil/1)
  # end
  ...
  defp args_to_opts(args) do
    ...
    ordered_slugs = order_by |> sort_slugs_by_order_by(included_slugs)
  ...
  defp sort_slugs_by_order_by(nil, slugs), do: slugs

  defp sort_slugs_by_order_by(order_by, :all) do
  ...
  defp sort_slugs_by_order_by(order_by, slugs) do
    %{metric: metric, from: from, to: to, aggregation: aggregation, direction: direction} =
      order_by
