defmodule SanbaseWeb.Graphql.Resolvers.ProjectResolver do
  ...
  def project_main_contract_address(%Project{id: id}, _args, %{
        context: %{loader: loader}
      }) do
    loader
    |> Dataloader.load(SanbaseDataloader, :contract_addresses, id)
    |> on_load(fn loader ->
      contract_addresses = Dataloader.get(loader, SanbaseDataloader, :contract_addresses, id)

      result =
        case contract_addresses do
          [_ | _] ->
            main = Project.ContractAddress.list_to_main_contract_address(contract_addresses)
            if main, do: main.address, else: nil
  ...
  def project_contract_addresses(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :contract_addresses, id)
    |> on_load(fn loader ->
      contract_addresses =
  ...
  def eth_addresses_resolver_fun(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
  ...
  def social_volume_query(%Project{id: id} = project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :social_volume_query, id)
    |> on_load(fn loader ->
      svq = Dataloader.get(loader, SanbaseDataloader, :social_volume_query, id)

      result =
        case svq do
          nil ->
            Project.SocialVolumeQuery.default_query(project)

          %{query: query} when query in [nil, ""] ->
            svq.autogenerated_query
  ...
  def source_slug_mappings(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :source_slug_mappings, id)
    |> on_load(fn loader ->
  ...
  def market_segment(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :market_segments, id)
    |> on_load(fn loader ->
      query = Dataloader.get(loader, SanbaseDataloader, :market_segments, id)
      list = List.wrap(query)
      {:ok, list |> Enum.map(& &1.name) |> List.first()}
    end)
  end

  def market_segments(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :market_segments, id)
    |> on_load(fn loader ->
      query = Dataloader.get(loader, SanbaseDataloader, :market_segments, id)
  ...
  def project_market_segment_tags(%Project{id: id}, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :market_segments, id)
    |> on_load(fn loader ->
      query = Dataloader.get(loader, SanbaseDataloader, :market_segments, id)
