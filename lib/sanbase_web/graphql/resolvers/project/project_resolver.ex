defmodule SanbaseWeb.Graphql.Resolvers.ProjectResolver do
  ...
  def market_segment(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :market_segments, project.id)
    |> on_load(fn loader ->
      market_segments =
        Dataloader.get(loader, SanbaseDataloader, :market_segments, project.id) || []

      first_segment = List.first(market_segments)
      {:ok, first_segment}
    end)
  end

  def market_segments(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :market_segments, project.id)
    |> on_load(fn loader ->
      market_segments =
  ...
  def main_contract_address(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :main_contract_address, project.id)
    |> on_load(fn loader ->
      main_contract_address =
        Dataloader.get(loader, SanbaseDataloader, :main_contract_address, project.id)

      {:ok, main_contract_address}
    end)
  end

  def contract_addresses(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :contract_addresses, project.id)
    |> on_load(fn loader ->
      contract_addresses =
        Dataloader.get(loader, SanbaseDataloader, :contract_addresses, project.id) || []

      {:ok, contract_addresses}
    end)
  end

  def eth_addresses(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :eth_addresses, project.id)
    |> on_load(fn loader ->
      eth_addresses =
        Dataloader.get(loader, SanbaseDataloader, :eth_addresses, project.id) || []

      {:ok, eth_addresses}
    end)
  end

  def source_slug_mappings(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :source_slug_mappings, project.id)
    |> on_load(fn loader ->
      source_slug_mappings =
        Dataloader.get(loader, SanbaseDataloader, :source_slug_mappings, project.id) || []

      {:ok, source_slug_mappings}
    end)
  end

  def social_volume_query(
        %Project{social_volume_query: %{query: _, autogenerated_query: _} = social_volume_query},
        _args,
        _resolution
      ) do
    {:ok, social_volume_query_to_string(social_volume_query)}
  end

  def social_volume_query(project, _args, %{context: %{loader: loader}}) do
    loader
    |> Dataloader.load(SanbaseDataloader, :social_volume_query, project.id)
    |> on_load(fn loader ->
      social_volume_query =
        Dataloader.get(loader, SanbaseDataloader, :social_volume_query, project.id)

      case social_volume_query do
        %{query: _, autogenerated_query: _} ->
          {:ok, social_volume_query_to_string(social_volume_query)}
  ...
  defp social_volume_query_to_string(%{query: q, autogenerated_query: aq}) do
    if q in [nil, ""], do: aq, else: q
  end
