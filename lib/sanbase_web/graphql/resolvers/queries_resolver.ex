defmodule SanbaseWeb.Graphql.Resolvers.QueriesResolver do
  ...
  def run_sql_query(
    ...
    with :ok <-
      ...
      Process.put(:queries_dynamic_repo, Queries.best_dynamic_repo_for_user(user.id))
  ...
  def compute_raw_clickhouse_query(
    ...
    run_raw_query_resolution(resolution, query_text, normalize_query_parameters(query_parameters))
  end
  ...
  def run_raw_sql_query(
    ...
    run_raw_query_resolution(
      resolution,
      query_text,
      normalize_query_parameters(query_parameters)
    )
  end

  defp normalize_query_parameters(params) do
    if params in [nil, "{}"], do: %{}, else: params
  end

  defp run_raw_query_resolution(resolution, query_text, query_parameters) do
    %{context: %{auth: %{current_user: user}} = context} = resolution
  ...
  def run_dashboard_sql_query(
    ...
    with :ok <- Queries.user_can_execute_query(user, product, plan),
         ...
         :ok <- Queries.process_put_best_dynamic_repo(user.id),
  ...
  def dashboard_comments_count(%{id: id}, _args, %{context: %{loader: loader}}) do
    ...
    |> on_load(fn loader ->
      count =
        ...
        |> SanbaseWeb.Graphql.DataFetchErrors.unwrap(:dashboard_comments_count, id)
