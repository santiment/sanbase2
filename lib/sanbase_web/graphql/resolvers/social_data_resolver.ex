defmodule SanbaseWeb.Graphql.Resolvers.SocialDataResolver do
  ...
  def get_most_tweets(_root, %{selector: selector, from: from, to: to, size: size, tweet_type: type}, _resolution) do
    SocialData.Tweet.get_most_tweets(selector, type, from, to, size)
  end

  def get_metric_spike_explanations(_root, %{metric: metric, slug: slug, from: from, to: to}, _resolution) do
  ...
  def get_metric_spikes_available_projects(_root, %{metric: metric}, _resolution) when is_binary(metric) do
    with false <- Sanbase.Metric.hard_deprecated?(metric),
  ...
  def get_trending_words(_root, %{from: from, to: to, interval: interval, size: size} = args, resolution) do
  ...
  def get_word_trending_history(_root, %{word: word, from: from, to: to, interval: interval, size: size}, _resolution) do
    SocialData.TrendingWords.get_word_trending_history(word, from, to, interval, size, :all)
  end

  def get_project_trending_history(_root, %{slug: slug, from: from, to: to, interval: interval, size: size}, _resolution) do
  ...
  def words_social_volume(
    ...
    SocialData.social_volume(selector, from, to, interval, :total, treat_word_as_lucene_query: treat_as_lucene)
  end

  def words_social_dominance(_root, %{selector: %{words: words}} = args, _resolution) do
    treat_as_lucene = Map.get(args, :treat_word_as_lucene_query, false)
  ...
  def words_context(_root, %{selector: %{word: word}, source: source, size: size, from: from, to: to}, _resolution) do
    size = Enum.min([size, 30])
    SocialData.word_context([word], source, size, from, to)
  end

  def words_context(_root, %{selector: %{words: words}, source: source, size: size, from: from, to: to}, _resolution) do
    size = Enum.min([size, 30])
    SocialData.word_context(words, source, size, from, to)
  end

  def word_context(_root, %{word: word, source: source, size: size, from: from, to: to}, _resolution) do
    size = Enum.min([size, 30])
    SocialData.word_context(word, source, size, from, to)
  end
  ...
  def word_trend_score(_root, %{word: word, source: source, from: from, to: to}, _resolution) do
