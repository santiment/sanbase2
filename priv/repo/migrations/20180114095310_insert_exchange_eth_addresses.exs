defmodule Sanbase.Repo.Migrations.InsertExchangeEthAddresses do
  ...
  defp update_or_create_eth_address({name, address, comments}) do
    ...
    |> case do
      nil ->
        ExchangeEthAddress.changeset(%ExchangeEthAddress{}, %{address: address, name: name, comments: comments})

      exch_address ->
        ExchangeEthAddress.changeset(exch_address, %{name: name, comments: comments})
    end
  end
