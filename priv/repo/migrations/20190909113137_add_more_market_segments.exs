defmodule Sanbase.Repo.Migrations.AddMoreMarketSegments do
  ...
  defp get_projects do
    Sanbase.Repo.preload(Project.List.projects(), [:infrastructure, :market_segments])
  end
  ...
  defp infrastrucutre_to_segment_id(projects) do
    ...
    infrastrucutre_to_segment =
      ...
      |> Enum.zip(~w(Bitcoin EOS Ethereum-Classic Ethereum IOTA Neo Nxt Omni Ubiq Waves Counterparty NEM))
      |> Map.new()
    ...
    market_segment_names = Map.values(infrastrucutre_to_segment)
    insert_data = Enum.map(market_segment_names, fn segment -> %{name: segment} end)
    ...
    market_segments = Sanbase.Repo.all(from(ms in MarketSegment, where: ms.name in ^market_segment_names))
    ...
    Map.new(infrastrucutre_to_segment, fn {k, v} ->
