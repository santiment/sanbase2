...
# The key and url helpers are public because test/scripts/
# fix_report_download_names_test.exs loads this file and exercises them. Nothing
# else calls into this module.
defmodule FixReportDownloadNames do
  ...
  defp copy(id, bucket, old_key, new_key) do
    case head_object(bucket, old_key) do
      {:ok, headers} ->
        copy_object(id, bucket, old_key, new_key, headers)

      # Nothing of the source survives `:REPLACE`, so a copy made without its
      # headers would be served as binary/octet-stream. The source was readable
      # when the plan was built - if it is not now, stop and say so.
      {:error, error} ->
        log(id, "aborted - could not HEAD the source #{old_key}: #{inspect(error)}")
        :error
    end
  end

  defp copy_object(id, bucket, old_key, new_key, headers) do
    opts =
      [
        ...
        # support conditional writes - the HEAD in ensure_destination_free/4
        # still guards, less tightly.
        if_none_match: "*"
      ]
  ...
  # points at. Only an S3 host names a bucket though: on a CDN or a waffle
  # `asset_host` url the first path segment is part of the key, so the bucket
  # has to come from the caller.
  #
  # `prefix` is what was taken off the path to get the bucket. `build_url/2`
  # puts it back, so key and url stay convertible for the path-style shape.
  defp split_url(url, fallback_bucket) do
    uri = URI.parse(url)
    host = uri.host || ""
    path = (uri.path || "") |> String.trim_leading("/") |> URI.decode()

    case String.split(host, ".s3", parts: 2) do
      # <bucket>.s3.amazonaws.com
      [bucket, _rest] when bucket != "" ->
        %{bucket: bucket, key: path, prefix: ""}

      # s3.amazonaws.com, s3.eu-central-1.amazonaws.com, s3-eu-west-1.amazonaws.com
      _no_bucket_in_host ->
        with true <- String.starts_with?(host, ["s3.", "s3-"]),
             [bucket, key] when key != "" <- String.split(path, "/", parts: 2) do
          %{bucket: bucket, key: key, prefix: bucket <> "/"}
        else
          _not_path_style -> %{bucket: fallback_bucket, key: path, prefix: ""}
        end
    end
  end

  def bucket_and_key(url, fallback_bucket) do
    %{bucket: bucket, key: key} = split_url(url, fallback_bucket)
    {bucket, key}
  end

  def build_url(old_url, new_key) do
    %{prefix: prefix} = split_url(old_url, nil)

    old_url
    |> URI.parse()
    |> Map.put(:path, "/" <> URI.encode(prefix <> new_key))
    |> URI.to_string()
  end
...
# Under `mix run` this is the whole point of the file. Under `mix test` the file
# is loaded for its helpers, and running the migration would be a surprise.
if Mix.env() != :test do
