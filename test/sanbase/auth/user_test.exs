defmodule Sanbase.Auth.UserTest do
  ...
  describe "#send_login_email when it user's first login" do
    setup do
      mock(Sanbase.MandrillApi, :send, {:ok, %{}})
      user = insert(:user, first_login: true)
      {:ok, user: user}
    end

    test "when user comes from Sheets", context do
      User.send_login_email(context.user, "https://sheets.santiment.net")
      assert_called(Sanbase.MandrillApi, :send, ["sheets-sign-up", _, _])
    end

    test "when user comes from Neuro", context do
      User.send_login_email(context.user, "https://neuro.santiment.net")
      assert_called(Sanbase.MandrillApi, :send, ["neuro-sign-up", _, _])
    end

    test "when user comes from other place (mainly sanbase)", context do
      User.send_login_email(context.user, "")
      assert_called(Sanbase.MandrillApi, :send, ["sanbase-sign-up", _, _])
    end
  end

  describe "#send_login_email when it not user's first login" do
    setup do
      mock(Sanbase.MandrillApi, :send, {:ok, %{}})
      user = insert(:user, first_login: false)
      {:ok, user: user}
    end

    test "when user comes from Sheets", context do
      User.send_login_email(context.user, "https://sheets.santiment.net")
      assert_called(Sanbase.MandrillApi, :send, ["sheets-sign-in", _, _])
    end

    test "when user comes from Neuro", context do
      User.send_login_email(context.user, "https://neuro.santiment.net")
      assert_called(Sanbase.MandrillApi, :send, ["neuro-sign-in", _, _])
    end

    test "when user comes from other place (mainly sanbase)", context do
      User.send_login_email(context.user, "")
      assert_called(Sanbase.MandrillApi, :send, ["sanbase-sign-in", _, _])
    end
  end
end
