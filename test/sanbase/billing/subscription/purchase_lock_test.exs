defmodule Sanbase.Billing.Subscription.PurchaseLockTest do
  ...
  setup context do
    ...
    enterprise_plan =
      insert(:plan_pro,
        ...
        is_private: false,
        is_deprecated: false,
        stripe_id: "plan_enterprise_year_" <> Ecto.UUID.generate()
      )

    user = insert(:user, stripe_customer_id: "cus_lock_" <> Ecto.UUID.generate())

    %{plan: plan, enterprise_plan: enterprise_plan, user: user}
  end
  ...
  describe "the Enterprise purchase flow" do
    test "a second concurrent purchase creates no Stripe subscription at all", context do
      %{user: user, enterprise_plan: plan} = context
    ...
    test "the lock does not stand in the way of a legitimate purchase", context do
      ...
      with_mocks(stripe_mocks()) do
        assert {:ok, subscription} = Subscription.subscribe(user, plan, "card_token")
        assert subscription.plan.name == "ENTERPRISE"
      end
    end

    test "a lock held for one user does not block a different user's purchase", context do
      %{user: user, enterprise_plan: plan} = context
      other = insert(:user, stripe_customer_id: "cus_other_" <> Ecto.UUID.generate())

      with_mocks(stripe_mocks()) do
        with_foreign_lock(other.id, fn ->
          assert {:ok, _} = Subscription.subscribe(user, plan, "card_token")
        end)
      end
    end
  end
