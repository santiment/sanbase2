defmodule Sanbase.Cryptocompare.HistoricalWorkerTest do
  ...
  use Oban.Testing, repo: Sanbase.Repo, prefix: "sanbase_scrapers"
  ...
  test "schedule work and scrape data", context do
    ...
    Sanbase.Mock.prepare_mock(HTTPoison, :get, fn url, _header, _ops ->
      ...
      data(base_asset, quote_asset, date_str)
    end)
    |> Sanbase.Mock.run_with_mocks(fn ->
      ...
      topic = state["asset_ohlcv_price_pairs"]

      # 10 days with 20 records in each
      assert length(topic) == 200

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609978320",
              "{\"base_asset\":\"#{base_asset}\",\"close\":6205.778967271624,\"high\":6206.719787115138,\"interval_seconds\":60,\"low\":6189.903645170898,\"open\":6189.526529700908,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609978320,\"volume_from\":63.31105002899999,\"volume_to\":392518.1490704502}"} in topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609546680",
              "{\"base_asset\":\"#{base_asset}\",\"close\":6208.7051725246765,\"high\":6209.396411564128,\"interval_seconds\":60,\"low\":6207.223419819188,\"open\":6209.099140044042,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609546680,\"volume_from\":11.45305277,\"volume_to\":71041.48491074912}"} in topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609804800",
              "{\"base_asset\":\"#{base_asset}\",\"close\":6250.308944432027,\"high\":6267.326355454034,\"interval_seconds\":60,\"low\":6240.674651886244,\"open\":6241.908080470284,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609804800,\"volume_from\":248.36219959,\"volume_to\":1553761.3212507297}"} in topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609891620",
              "{\"base_asset\":\"#{base_asset}\",\"close\":6208.461799608979,\"high\":6217.102504984712,\"interval_seconds\":60,\"low\":6206.278670187404,\"open\":6217.0036048556485,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609891620,\"volume_from\":45.82058862,\"volume_to\":284268.99153977534}"} in topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1610064120",
              "{\"base_asset\":\"#{base_asset}\",\"close\":6197.939996238564,\"high\":6242.094267553671,\"interval_seconds\":60,\"low\":6197.939996238564,\"open\":6242.400315239814,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1610064120,\"volume_from\":402.73847456,\"volume_to\":2496888.8341799136}"} in topic
    end)

    # Seems to fix some eror
    Sanbase.Cryptocompare.HistoricalScheduler.pause()
  end

  defp data(base_asset, quote_asset, date_str) do
