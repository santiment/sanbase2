defmodule Sanbase.Cryptocompare.HistoricalWorker.OHLCVPriceTest do
  ...
  test "schedule work and scrape data", context do
    ...
    |> Sanbase.Mock.run_with_mocks(fn ->
      ...
      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609978320",
              sort_json_keys(
                "{\"base_asset\":\"#{base_asset}\",\"close\":6205.778967271624,\"high\":6206.719787115138,\"interval_seconds\":60,\"low\":6189.903645170898,\"open\":6189.526529700908,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609978320,\"volume_from\":63.31105002899999,\"volume_to\":392518.1490704502}"
              )} in ohlcv_topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609546680",
              sort_json_keys(
                "{\"base_asset\":\"#{base_asset}\",\"close\":6208.7051725246765,\"high\":6209.396411564128,\"interval_seconds\":60,\"low\":6207.223419819188,\"open\":6209.099140044042,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609546680,\"volume_from\":11.45305277,\"volume_to\":71041.48491074912}"
              )} in ohlcv_topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609804800",
              sort_json_keys(
                "{\"base_asset\":\"#{base_asset}\",\"close\":6250.308944432027,\"high\":6267.326355454034,\"interval_seconds\":60,\"low\":6240.674651886244,\"open\":6241.908080470284,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609804800,\"volume_from\":248.36219959,\"volume_to\":1553761.3212507297}"
              )} in ohlcv_topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1609891620",
              sort_json_keys(
                "{\"base_asset\":\"#{base_asset}\",\"close\":6208.461799608979,\"high\":6217.102504984712,\"interval_seconds\":60,\"low\":6206.278670187404,\"open\":6217.0036048556485,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1609891620,\"volume_from\":45.82058862,\"volume_to\":284268.99153977534}"
              )} in ohlcv_topic

      assert {"cryptocompare_#{base_asset}_#{quote_asset}_1610064120",
              sort_json_keys(
                "{\"base_asset\":\"#{base_asset}\",\"close\":6197.939996238564,\"high\":6242.094267553671,\"interval_seconds\":60,\"low\":6197.939996238564,\"open\":6242.400315239814,\"quote_asset\":\"#{quote_asset}\",\"source\":\"cryptocompare\",\"timestamp\":1610064120,\"volume_from\":402.73847456,\"volume_to\":2496888.8341799136}"
              )} in ohlcv_topic
  ...
  defp sort_json_keys(json) do
    ...
    sorted_json =
      ~s({"close":#{map["close"]},"high":#{map["high"]},"low":#{map["low"]},"open":#{map["open"]},"timestamp":#{map["timestamp"]},"source":"#{map["source"]}","quote_asset":"#{map["quote_asset"]}","base_asset":"#{map["base_asset"]}","interval_seconds":#{map["interval_seconds"]},"volume_from":#{map["volume_from"]},"volume_to":#{map["volume_to"]}})

    sorted_json
  end

  defp sort_json_keys2(json) do
    map = Jason.decode!(json)

    sorted_json =
      ~s({"timestamp":#{map["timestamp"]},"source":"#{map["source"]}","quote_asset":"#{map["quote_asset"]}","base_asset":"#{map["base_asset"]}","price":#{map["price"]}})
