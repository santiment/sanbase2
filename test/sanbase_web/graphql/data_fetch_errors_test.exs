defmodule SanbaseWeb.Graphql.DataFetchErrorsTest do
    ...
    |> Sanbase.Mock.run_with_mocks(fn ->
      ...
      assert [
               %{
                 ...
                 "reason" => reason,
                 "keys" => keys,
                 "field" => "aggregatedTimeseriesData",
      ...
      expected_slugs =
        [context.p1, context.p2, context.p3] |> Enum.map(& &1.slug) |> Enum.sort()

      assert Enum.sort(keys) == expected_slugs
    end)
  end

  test "field alias is stored in dataFetchErrors entries", context do
    Sanbase.Mock.prepare_mock(
      Sanbase.Clickhouse.MetricAdapter,
      :aggregated_timeseries_data,
      fn _, _, _, _, _ -> {:error, "Something went wrong"} end
    ...
    |> Sanbase.Mock.run_with_mocks(fn ->
      response = execute(context.conn, "dev_activity_1d", alias: "devActivity")
      ...
      # The value is returned under the alias
      Enum.each(projects, fn project ->
        assert %{"devActivity" => nil, "slug" => _} = project
      end)

      # The error entry references the alias, not the field name
      assert [%{"source" => "aggregated_metric", "field" => "devActivity"}] =
               get_in(response, ["extensions", "dataFetchErrors"])
    end)
  end
  ...
  defp execute(conn, metric, opts \\ []) do
    field_alias =
    ...
    {
      allProjects {
        #{field_alias}aggregatedTimeseriesData(
          metric: "#{metric}"
