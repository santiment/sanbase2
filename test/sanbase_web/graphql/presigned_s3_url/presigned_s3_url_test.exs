defmodule SanbaseWeb.Graphql.PresignedS3UrlTest do
  ...
  alias Sanbase.PresignedS3Url.S3
  ...
  test "get a presigned S3 url", context do
    (&PresignedS3Url.S3.generate_presigned_url/3)
    |> Sanbase.Mock.prepare_mock2({:ok, @url})
    |> Sanbase.Mock.run_with_mocks(fn ->
      url =
        context.conn
        |> get_presigned_s3_url("dataset")
  ...
  test "presigned url S3 expires", context do
    ...
    dt = DateTime.add(DateTime.utc_now(), PresignedS3Url.expires_in() + 10, :second)

    dt_mock = Sanbase.Mock.wrap_consecutives([fn -> now end, fn -> dt end], arity: 0)

    S3
    |> Sanbase.Mock.prepare_mock(:utc_now, dt_mock)
